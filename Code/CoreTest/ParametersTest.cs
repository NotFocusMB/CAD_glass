...
namespace CoreTest
{
    ...
    public class ParametersTest
    {
        ...
        [Test(Description = "Проверяет, что конструктор правильно" +
        ...
        [Test(Description = "Проверяет, что SetDependencies правильно" +
                            " обновляет значение")]
        public void SetDependencies_ShouldChangeParameterValue()
        {
            _parameters.SetDependencies(40.0, ParameterType.BowlRadius);
            _parameters.NumericalParameters[ParameterType.BowlRadius]
        ...
        [Test(Description = "Проверяет, что ResetToDefaults возвращает" +
                            " все к 'заводским' настройкам")]
        ...
        [Test(Description = "Проверяет, что SetDependencies пробрасывает" +
                            " исключение от Parameter.Value")]
        public void SetDependencies_WhenValueIsInvalid_ShouldThrowArgumentException()
        {
            Action act = () => _parameters.SetDependencies(200.0,
                ParameterType.StalkHeight);

            act.Should().Throw<ArgumentOutOfRangeException>().WithMessage(
                "Значение (200) не может быть больше максимального (100).*",
                "потому что значение выходит за 'заводские' рамки");
        }

        [Test(Description = "Проверяет, что зависимости не ломаются" +
                            " при нулевом угле")]
        public void UpdateAllLimits_WhenAngleIsZero_ShouldNotLimitHeight()
        {
            ...
            var sideHeightParam =
                _parameters.NumericalParameters[ParameterType.SideHeight];

            sideHeightParam.MaxValue.Should().Be(50.0);
        }

        [Test(Description = "Проверяет, что GetLimitsString возвращает N/A" +
